class Passport
  ...
  def initialize(passport_data)
    self.passport_data = passport_data.gsub("\n", ' ').split(' ').map { |data| data.split(':') }.to_h
  end

  def valid?
    return false unless (REQUIRED_FIELDS - passport_data.keys).empty?
    return false unless (1920..2002).cover?(byr.to_i)
    return false unless (2010..2020).cover?(iyr.to_i)
    return false unless (2020..2030).cover?(eyr.to_i)
    return false unless (150..193).cover?(height_in_cm)
    return false unless hcl.downcase.match(/^#[0-9a-f]{6}$/)
    return false unless VALID_EYE_COLORS.include?(ecl)
  ...
  def method_missing(method_name, *args)
    passport_data[method_name.to_s]
  end

  def respond_to_missing?(method_name, include_private = false)
    super || passport_data.keys.include?(method_name)
  end

  def height_in_cm
    height_unit == 'cm' ? hgt.to_i : (hgt.to_i * 2.54).round.to_i
  end
  ...
  def to_s
    passport_data.merge(valid: valid?)
  end
end

passport_data = DATA.read.split("\n\n")
passports = passport_data.map { |data| Passport.new(data) }
pp passports.map(&:to_s)
valid_passports = passports.select(&:valid?)
pp valid_passports.count
