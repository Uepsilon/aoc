bag_rules = DATA.read.gsub(/bags?/, '').split("\n")
MY_BAG_COLOR = 'shiny gold'.freeze

class BagFinder
  attr_accessor :grouped_bag_rules, :including_bag_count_map

  def initialize(bag_rules)
    self.grouped_bag_rules = bag_rules.each_with_object({}) do |input, rules|
      bag_color, bag_rule = input.split('contain').map(&:strip)
      rules[bag_color] = bag_rule.scan(/(\d)\s([\w\s]+\w)*/).map(&:reverse).to_h
    end
    self.including_bag_count_map = {}
  end

  def find_potential_bags_bearer(potential_bags)
    potential_bags = [potential_bags] unless potential_bags.is_a?(Array) # Array.wrap <3
    new_potential_bags = grouped_bag_rules
                          .reject { |bag_color, bag_rules| (bag_rules.keys & potential_bags).empty? }
                          .keys
                          .concat(potential_bags)
                          .uniq

    if potential_bags.size < new_potential_bags.size
      find_potential_bags_bearer(new_potential_bags)
    else
      new_potential_bags
    end
  end

  def bag_count_in(bag_color)
    bag_sum = grouped_bag_rules[bag_color].sum do |bag_color, bag_count|
      if including_bag_count_map.key?(including_bag_count_map)
        including_bag_count_map[bag_color]
      else
        bag_count.to_i * bag_count_in(bag_color)
      end
    end

    # must be +1 because we have to count the bag itself
    self.including_bag_count_map[bag_color] = bag_sum + 1
    including_bag_count_map[bag_color]
  end
end
